...
| `TRANSCRIPT_RETENTION_DAYS` | Days to keep raw transcripts before auto-cleanup | `30` |
| `API_KEY` | API key for `/trigger` endpoints (optional) | |
| `PORT` | Server port | `3000` |
| `DATA_DIR` | Data directory for journals, transcripts, and user config | `./data` |
...
4. When the call ends, the raw transcript is saved to `data/transcripts/<user-id>/YYYY-MM-DD.txt`
5. The transcript is summarized into a first-person journal entry
6. The journal is saved as markdown in `data/journals/<user-id>/YYYY-MM-DD.md`
7. Multiple calls on the same day are appended to the same file
...
## Transcripts and Reprocessing

Raw transcripts are automatically saved for every call to `data/transcripts/<user-id>/YYYY-MM-DD.txt`, even if the journal was skipped (callback, voicemail, etc.). This lets you reprocess entries later if the summarization prompt improves or a journal was missed.
...
npm run reprocess -- --list alice

# Reprocess a transcript into a new journal entry (overwrites existing)
npm run reprocess -- 2026-04-06

# Reprocess for a specific user
npm run reprocess -- alice 2026-04-06
```

Transcripts older than 30 days are automatically cleaned up on server startup. Configure with `TRANSCRIPT_RETENTION_DAYS` in `.env`.
